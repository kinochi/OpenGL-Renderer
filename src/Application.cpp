...
int main()
{
	...
	// By default the Vertex array attributes are disabled so we need to enable them
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER,6 * 2 * sizeof(unsigned int),indices,GL_STATIC_DRAW);	// Has to be unsigned ( be it char or int)

	struct ShaderProgramSource shaders = ParseShader("../res/shaders/Basic.shader");
	/*
	 *std::cout << "VERTEX:" << "\n";
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		glDrawElements(GL_TRIANGLES,3*2, GL_UNSIGNED_INT, nullptr);	// total of 2 triangle with 3 positions per triangle 
